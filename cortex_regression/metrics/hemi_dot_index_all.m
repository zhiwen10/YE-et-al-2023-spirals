%%
load('hemi_weights_8points_allsessions.mat');
load('axon_intensity_all_hemi.mat');
...
figHand = figure('Renderer', 'painters', 'Position', [100 100 500 400]);
axx4 = subplot(1,4,1)
...
set(gca,'Ydir','reverse')
axis image; axis off
print(figHand, 'hemi_mean_activity_axon', '-dpdf', '-bestfit', '-painters');
%%
for i = 1:15
...
%%
save('hemi_dot_index_permutation.mat','dot_all','h1','p');
%%
h = figure('Renderer', 'painters', 'Position', [100 100 300 500]);
perm_index = ones(size(1001,1));
for i = 1:15
...
print(h, 'hemi_dot_index_permutation', '-dpdf', '-bestfit', '-painters');
%%
clear dot_perm_all2
dot_real = 0;
for kkk = 1:8
...
xlim([0,1.2]);
%%
print(h, 'hemi_mean_dot_index_permutation', '-dpdf', '-bestfit', '-painters');