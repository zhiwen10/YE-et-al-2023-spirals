...
for kk = 1:15
    ...
    load([savefolder fname '-hemi.mat'],'kernel_full2');
    %% color overlay a cycle
    for kkk = 1:8
        TheColorImage_all(:,:,kkk,kk) = squeeze(kernel_full2(:,:,point(kkk,1),point(kkk,2)));        
    end
end
%% dot product
for i = 1:8
...
for kk = 1
    figure;
    for kkk = 1:8
        gcamp = squeeze(TheColorImage_all(:,:,kkk,kk));
...
%%
intensity_all1 = intensity_all1+min(intensity_all1(:));
for kk = 1:15
    for kkk = 1:8
        gcamp = squeeze(TheColorImage_all(:,:,kkk,kk));
        for i = 1:8
            axon = squeeze(intensity_all1(:,:,i));
            dot_a(kkk,i,kk) = dot(gcamp(:),axon(:));
        end
    end
end
dot_a1 = mean(dot_a,3);
