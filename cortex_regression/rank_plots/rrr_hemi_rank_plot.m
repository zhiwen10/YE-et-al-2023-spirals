...
addpath(genpath('C:\Users\Steinmetz lab\Documents\MATLAB\YE2023\cortex_regression'));
%%
...
kk1 = b(:,n) *a(:,n)';
k1_real = Uleft*kk1;
kernel_full = zeros(size(BW_empty,1)*size(BW_empty,2),size(BW_empty,1)*size(BW_empty,2));
...
kernel_full2 = reshape(kernel_full,[size(BW_empty,1),size(BW_empty,2),...
    size(BW_empty,1),size(BW_empty,2)]);
%% rrr regressor 2d rank
% kk1 = b(:,n) *a(:,n)';
kk1_dim = Uleft*b(:,1:50);
kk1_full_dim = zeros(size(BW_empty,1)*size(BW_empty,2),50);
for i = 1:numel(indexleft)
    kk1_full_dim(indexleft(i),:) = kk1_dim(i,:);
end
kk1_full_dim = reshape(kk1_full_dim,[size(BW_empty,1),size(BW_empty,2),size(kk1_full_dim,2)]);
%% rrr target rank
kk2_full_dim = zeros(size(BW_empty,1)*size(BW_empty,2),50);
for i = 1:numel(indexright)
    kk2_full_dim(indexright(i),:) = a(i,:);
end
kk2_full_dim = reshape(kk2_full_dim,[size(BW_empty,1),size(BW_empty,2),size(kk2_full_dim,2)]);
%%
...
print(h1, 'KernelMap-hemisphere-individual', '-dpdf', '-bestfit', '-painters');
...
%%
print(h1, 'time-series-dimension-projection-hemi', '-dpdf', '-bestfit', '-painters');