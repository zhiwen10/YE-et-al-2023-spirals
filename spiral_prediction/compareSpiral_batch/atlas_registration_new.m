...
%%
tform_folder = 'C:\Users\Steinmetz lab\Documents\MATLAB\widefield_DIY\phase\reducedRankRegression\registration';
% for kk = 1:size(T1,1)
for kk = 28
    % clearvars -except T1 kk params 
    clear  cpstruct movingPoints fixedPoints rform
    ops = get_session_info(T1,kk);
    fname = [ops.mn '_' ops.tdb '_' num2str(ops.en)];
    filename = fullfile(tform_folder,[fname '_tform.mat']);
%     if isfile(filename)
%         copyfile(filename);
%     else   
