function hs6ab = plotSpiralDensityByRadius(data_folder,save_folder)
%%
...
hist_bin = 40;
count1 = 1;
hs6ab = figure('Renderer', 'painters', 'Position', [100 100 1300 700]);
for k  = 1:7
    clear unique_spiral_unit
    radius_i = radius(k);
    load(fullfile(data_folder,'spirals','spirals_density_radius',...
        ['histogram_' num2str(radius_i) 'radius.mat']));                   % load histogtam counts
    ...
    set(cb1,'Position',...
        [cb_size1(1)+0.05  cb_size1(2) cb_size1(3) cb_size1(4)/2]);        % To change size
    title([num2str(radius(k)) ' mm']);
    %% draw a line 
    x = [105,520]; y = [780,225];
    xq = 105:520;
    vq = interp1(x,y,xq);
...
print(hs6ab, fullfile(save_folder,'Figs6ab_spirals_density_radius.pdf'),...
