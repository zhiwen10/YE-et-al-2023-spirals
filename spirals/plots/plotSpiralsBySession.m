function hs5 = plotSpiralsBySession(T,session_rows,data_folder,save_folder)
...
%%
hs5 = figure('Renderer', 'painters', 'Position', [100 100 1100 900]);
count1 = 1;
for kk = session_rows                                                      % plot 6 session in one figure, to avoid crowding
    ...
    ax0(count1) = subplot(2,6,count1);                                     % plot registered sign map with brain outline
    ...
    ax1(count1) = subplot(2,6,6+count1);
...
fig_name = ['spirals_by_session_' num2str(session_rows(1)) '_' ,...
    num2str(session_rows(end))];
print(hs5,fullfile(save_folder,fig_name),...
