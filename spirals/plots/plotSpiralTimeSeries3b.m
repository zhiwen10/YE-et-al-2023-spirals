function h1ac = plotSpiralTimeSeries3b(data_folder,save_folder)
...
yscale = 150;
for i = 1:size(pixel,1)
    plot(ax3,qt4,trace_raw(i,frame0:last_frame1)*yscale-3*(i-1),...
        'Color',color3(i,:),'lineWidth',1);
    hold on; 
    plot(ax3,qt4,trace_filt(i,frame0:last_frame1)*yscale-3*(i-1),'Color',color2(i,:),'lineWidth',2);
...
yscale = 150;
for i = 1:size(pixel,1)
    plot(ax4,qt4,trace_filt(i,frame0:last_frame1)*yscale,...
        'Color',color2(i,:),'lineWidth',2);
    hold on;
end
ax4.FontSize = 8; 
set(ax4,'YTickLabel',[]);
...
xlim([-0.1,0.25]);
%%
print(h1ac,fullfile(save_folder, 'Fig1ac_overlay2.pdf'),...
