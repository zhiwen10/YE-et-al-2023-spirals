function hs15gh = plotSpiralTrialExample2(data_folder, save_folder)
...
% indx = [576,623,608]; % 3 spirla example, 3 no spiral example 
% indx = [576,623,582,588];
indx = [576,582];
cmax = 0.02; cmin = -0.02;
...
for kk = 1:numel(indx)
    ...
    trace = Ut1a*dV1(:,indx_t-10:indx_t+32);
    ...
    row = kk;
    for i = 1:14
        ax4 = subplottight(4,15,(row-1)*15+i);
        im1 = imagesc(squeeze(trace2d(:,:,i+8)));
        hold on;
        ...
        caxis([cmin,cmax]);
        %%
        ax5 = subplottight(4,15,(row+1)*15+i);
        im1 = imagesc(squeeze(tracePhase(:,:,i+8)));
        hold on;
        plotOutline(maskPath([1:11]),st,atlas1,hemi,scale3,lineColor);
        axis image; axis off;
        set(im1, 'AlphaData', BW3, 'AlphaDataMapping', 'scaled');
...
im1 = imagesc(squeeze(trace2d(:,:,15)),'visible','on');
hold on;
plotOutline(maskPath([1:11]),st,atlas1,hemi,scale3,lineColor);
axis image; axis off;
set(im1, 'AlphaData', BW3, 'AlphaDataMapping', 'scaled');
...
%%
print(hs15gh,fullfile(save_folder,'FigS15gh_example_spiral_nospiral4')...
