function h5c = plotCorrectMeanTrace2(data_folder,save_folder)
...
hold on;
scatter(center(2),center(1),16,'w');
%
for kk = 1
    ...
    for i = 1:8
        ...
        plot(ax2,qt,trace_raw(i,:)*scalea*100+2*(i-1),'color',color2(i+2,:),'lineWidth',1);
        hold on; 
        [pks,locs] = findpeaks(trace_raw(i,:),'MinPeakProminence',0.0005);
    ...
    set(ax2,'YTickLabel',[]);
    for i = 1:8
        text(qt(311)-0.1,(i-1)*2,nameList{i},'color','k','Interpreter','None');
    end
...
%%
print(h5c,fullfile(save_folder,'Fig5c_task_correct_mean_trace4.pdf'),'-dpdf', '-bestfit', '-painters');