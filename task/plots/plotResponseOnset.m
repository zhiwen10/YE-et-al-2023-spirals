...
for m = 1:4
    ...
    for kk = 1:5
        ...
        trace_task_contra{kk,m} = trace_all3_conrta;
        %%
        ...
        trace_task_ipsi{kk,m} = trace_all3_ipsi;
    end
end
%%
trace = trace_task_ipsi{5,1};
trace_mean = mean(trace,2);
figure;
plot(-4:1/35:4, trace_mean);
%%
trace_passive_contra = [];
trace_passive_ipsi = [];
for m = 1:4
    ...
    for kk = 1:5
        ...
        trace_passive_contra{kk,m} = trace_all_contra;
        %%
        trace_all_ipsi = squeeze(wf_all(:,1,index));
        trace_passive_ipsi{kk,m} = trace_all_ipsi;        
    end
end
%%
t1 = -4:1/35:4;
trace = cat(2,trace_passive_contra{5,:});
trace_ipsi = cat(2,trace_passive_ipsi{5,:});
%%
trace_mean = mean(trace,2);
figure;
plot(-4:1/35:4, trace_mean);
%%
% response = trace(146,:)-trace(141,:);
response = trace(146,:)-trace_ipsi(146,:);
value_onset = trace(141,:);
figure;
scatter(value_onset,response);
%%
...
subplot(2,4,2);
for i = 1:5
    trace_temp = squeeze(trace_passiven_ipsi(:,i,:));
