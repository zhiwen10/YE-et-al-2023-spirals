...
flow_folder = 'C:\Users\Steinmetz lab\Documents\git\BIL\function_matching\spirals_all';
for kk = 1:15
    ...
    load(fullfile(flow_folder,[fname '_mean_phase_flow.mat']),'vxRaw_mean','vyRaw_mean');
    vxRaw = vxRaw_mean; vyRaw = vyRaw_mean; 
    %%
    ...
    [h1(kk,1),p(kk,1)] = ttest2(sum_dot,sum_dot_perm_all);
    %%
...
%     axis image; axis off;
    ...
    [h2(kk,1),p2(kk,1)] = ttest2(sum_dot1,sum_dot_perm_all1);
%     ax2 = subplot(1,3,3)
%     histogram(sum_dot_perm_all1);
%     hold on;
%     xline(sum_dot1);
%     text(1,120,num2str(p2(kk)));
%     text(1,130,num2str(sum_dot1));
%     ylim([0,150]);
%     print(h3, [fname '_matching_index'], '-dpdf', '-bestfit', '-painters');
end