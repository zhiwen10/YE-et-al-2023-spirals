%% Cross-Frequency Phase-Amplitude Coupling Analysis
% Testing PAC between 0.1-0.5Hz (phase) and 2-8Hz (amplitude)
...
signal = double(trace)';
% Frequency bands
low_freq_band = [0.01, 1]; % Phase-providing frequency (very slow oscillation)
...
% Design filters for frequency bands - optimized for very low frequencies
% Low frequency (phase) - lower order filter for very slow oscillations
[b_low, a_low] = butter(2, low_freq_band/(fs/2), 'bandpass');
...
phase_freqs = 0.01:0.02:1.0;    % Phase frequencies (very slow to slow)
...
for i = 1:length(phase_freqs)
    for j = 1:length(amp_freqs)
        % Filter for current frequency pair - adjusted for very low frequencies
        phase_bandwidth = max(0.01, phase_freqs(i) * 0.2); % Adaptive bandwidth
        [b_p, a_p] = butter(2, [max(0.005, phase_freqs(i)-phase_bandwidth), ...
                               min(phase_freqs(i)+phase_bandwidth, 1.5)]/(fs/2), 'bandpass');
...
hold on;
bar(x_pos, surrogate_means, 'r', 'Alpha', 0.5);
