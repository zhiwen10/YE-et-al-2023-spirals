function h5e = plotSpiralRateAllRadius(data_folder,save_folder)
%%
...
for i = 1:4
    ...
    for m = 1:numel(Sessions)
        ...
        spiral_count_pre(m,:) = squeeze(sum(spiral_count_sum_temp(:,frames_pre),2))/numel(frames_pre);
        spiral_count_post(m,:) = squeeze(sum(spiral_count_sum_temp(:,frames_post),2))/numel(frames_post);
    end
    %%
...
%%
print(h2,'spirals_comparison_all_radius.pdf','-dpdf', '-bestfit', '-painters');
%%
for k = 1:3
