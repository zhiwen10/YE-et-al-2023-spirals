function T = getAxonBiasTableLayer(data_folder,save_folder)
...
i = 3;
ilayer = layer{i};
region_test = cellfun(@(x) {append(x,ilayer)},sensory_label);
sensory_label2 = cat(2,regions_all,region_test);
%%
for k = 1:numel(sensory_label2)
    iregion = sensory_label2(k);
    st_region_indx = getRegionIndex2(iregion,st);                          % get brain region index
...
for i = 1:size(soma_all4,1)
    ...
    names(i,1) = st{st.index == indxa(i,1),4};
end
%%
pat = ["1","2/3","4","5","6"];
for i = 1:5
...
T = table(soma_center,soma_angle,z,axon_vector,...
    angle1,polarity,ang_diff2,labels_all, TF1, TF2, TF3);
T = renamevars(T,["z","axon_vector","angle1","polarity","ang_diff2","TF1","TF2","TF3"], ...
                 ["soma_polar_angle","axon_bias","bias_angle",...
                 "pc_ratio","center_bias_angle","layer1/4","layer5","layer6"]);
%%
