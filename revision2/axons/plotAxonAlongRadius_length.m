function [h2ac] = plotAxonAlongRadius_length(data_folder,save_folder)
%%
...
lm = fitlm(distance,T1.axon_length);
% hold on;
plot(lm);
...
ax5 = subplot(2,3,5);
coef1 = corrcoef(distance,T1.axon_length);
coef1 = coef1(1,2);
data_n = length(distance);
for i = 1:5000
    pa = randperm(data_n);
    distance1 = distance(pa);
    corr_temp = corrcoef(distance1,T1.axon_length);
...
for i = 1:length(radius_bins)-1
    ...
    pc_ratio_temp = T1.axon_length(pc_ratio_index);
...
ylim([0,500]);
print(h2ac, fullfile(save_folder,'Fig2ac_axon_bias_all2_axon_length.pdf'),...
