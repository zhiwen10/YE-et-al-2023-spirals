...
SSp_list = SSp_list2;
%%
load('cutting_SSp_all.mat');
%%
h1 = figure('Renderer', 'painters', 'Position', [100 100 900 300]);
subplot(1,2,1);
for i = 1:3
    data_temp = MO_list(:,i);
    scatter(ones(size(data_temp))*i,data_temp,8,'k');
    hold on;
end
for i = 1:2
    data_temp1 = MO_list(:,i);
    data_temp2 = MO_list(:,i+1);
    plot([ones(size(data_temp1))*i,ones(size(data_temp2))*(i+1)]',...
        [data_temp1,data_temp2]','color','k');
end

xticks([1:4]);
xticklabels({'Control','Cranitomy','Cutting'});
...
subplot(1,2,2);
for i = 1:3
    data_temp = SSp_list(:,i);
    scatter(ones(size(data_temp))*i,data_temp,8,'k');
    hold on;
end
for i = 1:2
    data_temp1 = SSp_list(:,i);
    data_temp2 = SSp_list(:,i+1);
    plot([ones(size(data_temp1))*i,ones(size(data_temp2))*(i+1)]',...
        [data_temp1,data_temp2]','color','k');
end
xticks([1:4]);
...
title('SSp spirals');
%%
[h1,p1] = ttest(MO_list(:,1),MO_list(:,3));
%%
print(h1, 'Cutting_SSp_summary_scatter', '-dpdf', '-bestfit', '-painters');    