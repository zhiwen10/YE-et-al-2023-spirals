function getSpiralsPhaseMapLeft(T,data_folder,save_folder)
...
for kk = 8:15
    ...
    filteredSpirals1(filteredSpirals1(:,4)==0,4) = -1;
    filteredSpirals1 = filteredSpirals1(filteredSpirals1(:,4)==-1,:);
    location_index = (filteredSpirals1(:,1)>=250 &...
        filteredSpirals1(:,1)<=350 & ...
    ...
    for i = 1:spiral_n
        ...
        if frameTemp(1)>0 & frameTemp(end)<size(dV,2)
            dV1 = dV(:,frameTemp);
            ...
            spiral_phase_all(:,:,:,count1) = tracePhase_current;   
            count1 = count1+1;
        end
    end
    %% normalize all sprial phase maps to the same value at point (70,95) 
    spiral_phase_all_norm = zeros(size(spiral_phase_all));
    for i = 1:size(spiral_phase_all,4)
        spiral_phase_temp = squeeze(spiral_phase_all(:,:,:,i));
        spiral_phase_temp1 = spiral_phase_temp-spiral_phase_temp(70,48);
        spiral_phase_temp2 = angle(exp(1i*(spiral_phase_temp1))); 
        spiral_phase_all_norm(:,:,:,i) = spiral_phase_temp2;
    end
    %%
    save(fullfile(save_folder,[fname '_mean_flow2_left']),...
