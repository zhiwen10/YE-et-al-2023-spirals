...
%%
load(fullfile(data_folder,'revision2','spirals_compare',...
    'spiral_compare_sessions_neighbor.mat'));
%% check prediction
kk = 7;
spiral_left_match = spiral_left_match_all{7};
%%
mn = T.MouseID{kk};
tda = T.date(kk);
en = T.folder(kk);    
...
%%
predict_folder = 'E:\spiral_data_share\data\revision2\V_prediction';
load(fullfile(predict_folder, [fname '_v_predict.mat']));
%%
...
%%
point = point*8;
point(:,2) = 1140-point(:,2);
for i = 1:8
    traces(:,i) = squeeze(Utransformed(point(i,1),point(i,2),:))'*V;
    trace_predict(:,i) = squeeze(Utransformed(point(i,1),point(i,2),:))'*V_predict;
end
%%
tn = size(trace_predict,1);
figure;
for i = 1:8
    plot(t(1:tn),traces(1:tn,i)+1500*i,'k');
    hold on;
    plot(t(1:tn),trace_predict(1:tn,i)+1500*i,'r');
    hold on;
end
%%
params.downscale = 8;
...
tStart = 1240; tEnd = 1245; % find spirals between time tStart:tEnd
...
ax4 = subplottight(4,subplotn,10);
im_raw = imagesc(dff(:,:,frame));
colormap(ax3, parula)
axis image; axis off;
set(im_raw, 'AlphaData', BW2, 'AlphaDataMapping', 'scaled');
