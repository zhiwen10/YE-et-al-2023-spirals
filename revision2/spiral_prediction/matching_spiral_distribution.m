...
for kk = 1:15
    ...
    [MatchPointSL,MatchPointSR,DirDiffc] = UniqueMatchPoints(pointSL,pointSR);   
    %%
    MatchPointSL_all = [MatchPointSL_all;MatchPointSL];
    MatchPointSR_all = [MatchPointSR_all;MatchPointSR];
end
%%
fullXSize = 1200;
symetryCriteria = 100;
xcheck1 = MatchPointSL_all(:,1)+MatchPointSR_all(:,1); 
ind1 = find(xcheck1>=(fullXSize-symetryCriteria) & xcheck1<=(fullXSize+symetryCriteria));
ycheck1 = MatchPointSL_all(:,2)-MatchPointSR_all(:,2); 
ind2 = find(ycheck1>=-symetryCriteria & ycheck1<=symetryCriteria);
ind = intersect(ind1,ind2);
MatchPointSL_all = MatchPointSL_all(ind,:); MatchPointSR_all = MatchPointSR_all(ind,:); 
%%
MatchPointSLR_all = [MatchPointSL_all;MatchPointSR_all];
...
[unique_spirals,scolor,low_color_bound,high_color_bound] = ...
    density_color_plot(MatchPointSLR_all,hist_bin);
%%
h1e = figure;
hist_bin = 40;
