function getReducedRankRegressionHEMI_simple_test(T,data_folder,save_folder)
...
for kk = 1:15
    ...
    data = UselectedLeft*V(:,1:20000);   
    % subtract mean as we did before
    data = data-mean(data,2);
    regressor_test = Uleft' * data;   
    regressor_test_z = zscore(regressor_test,[],2);
    traceSSp2 = UselectedLeft*V(:,1:20000);
    traceSSp_predict = regressor_test_z'*kk1;
    ...
    for m = 1:numel(shift_t)
        ...
        % traceSSp_predict_temp = squeeze(traceSSp_predict(m,:,:));
        explained_var(m,1) = sseExplainedCal(traceSSp2_shift(:)',traceSSp_predict_temp(:)');
        % explained_var(m,1) = sseExplainedCal(traceSSp2(:)',traceSSp_predict_temp(:)');
    end
    %%
    save(fullfile(save_folder,[fname '-hemi_simple.mat']),'explained_var','shift_t');
    close all;
end