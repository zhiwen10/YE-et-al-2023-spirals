...
% save_folder = 'C:\Users\Steinmetz lab\Documents\MATLAB\widefield_DIY\phase\compareFlowField\compare_flow_new1';
% roi_folder = 'C:\Users\Steinmetz lab\Documents\MATLAB\widefield_DIY\phase\compareSpiral\compareSpiral_batch\roi';
rfolder = 'C:\Users\Steinmetz lab\Documents\MATLAB\widefield_DIY\phase\compareFlowField\compare_flow_new1\flow_var_threshold_01';
%%
color1 = {'g','r','m'};
sig_ratio = cell(3,1);
...
for current_area = [1,2,4]
    %%
    indx = find(contains(T1.Area,area{current_area}));
    current_T = T1(indx,:);
    ...
    for kk = list
        ...
        edges1(nan_index) = [];
        % edges1 = edges(1,2:size(flow_var1,2)+1);
        ...
        pp = anovan(flow_var3,{edges_matrix2 permute_id_matrix2},'model',2,'varnames',{'amp','permute'},'display','off');
        pp_all = [pp_all;pp'];
...
%%
print(h1, 'all_scramble_new', '-dpdf', '-bestfit', '-painters');
%%
figure;
for i = 1:3
    scatter(flow_var_all{i},sig_ratio{i})
    hold on;
end
%%
count2  =1;
...
%%
color2 = {'g','r','c','m'};
h1 = figure('Renderer', 'painters', 'Position', [100 100 900 600]);
count1 = 1;
for current_area = [1,2,4]
    subplot(1,3,count1);
    ...
    pp2 = anovan(flow_2way3,{edges_matrix3 permute_id_matrix3},'model',2,'varnames',{'amp','permute'});
    ...
    for ii = 1:8
        ...
        % flow_real_temp1(isnan(flow_real_temp1)) = [];
        flow_perm_temp1 = flow_perm_temp2(:,ii); 
        % flow_perm_temp1(isnan(flow_perm_temp1)) = [];
        % [h(ii),p(ii)] = ttest(flow_real_temp1,flow_perm_temp1,"Tail","right");
...
%%
print(h1, 'flow_matching_sumamry_new', '-dpdf', '-bestfit', '-painters');
%%
count2  =1;
for current_area = [1,2,4]
    %%
...
for current_area = [1,2,4]
    ...
    errorbar(edges(1,1:8),phase_real_mean(1:8),phase_real_std(1:8),'color',color2{current_area});
    hold on;
    ...
    pp2 = anovan(phase_2way3,{edges_matrix3 permute_id_matrix3},'model',2,'varnames',{'amp','permute'});
    pp_all_area(:,count1) = pp2;
    %%
    % indx1 = not(isnan(flow_var1(1,:)));
    h = []; p = [];
...
print(h1, 'phase_matching_sumamry_new', '-dpdf', '-bestfit', '-painters');