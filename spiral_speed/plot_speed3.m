function h1 = plot_speed3(angular_velocity_all,linear_velocity_all,radiusi)
spiral_n = size(angular_velocity_all,1);
angular_velocity_per_spiral = nan(spiral_n,12); % maximum length is 12
h1 = figure('Renderer', 'painters', 'Position', [100 100 1100 250]);
ax1 = subplot(1,4,1);
p = randperm(spiral_n,5000);
scale1 = 8;
...
% for i = 1:distance_offset_per_spiral
%     plot(sampling_radius,linear_velocity_all(i,:),'-o','color',[0.5,0.5,0.5],...
%         'MarkerSize',3,'MarkerFaceColor',[0.5,0.5,0.5],'MarkerEdgeColor','none');
%     hold on;
% end
for i = 1:5000
...
% b1 = histogram(linear_velocity_all_max);
% [M, I] = max(b1.Values);
% max_distance = b1.BinEdges(I);
max_distance = mean(linear_velocity_all_max);
std_distance = std(linear_velocity_all_max);
edges2 = edges*radiusi*pixSize;
