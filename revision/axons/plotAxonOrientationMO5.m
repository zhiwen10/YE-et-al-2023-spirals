...
%%
T = readtable(fullfile(data_folder,'Revision','axons','Axon_bias_all_cells_MO2.csv'));
figure;
subplot(1,3,1)
T1 = T(T.soma_center_1<center(1),:);
histogram(T1.bias_angle+90,12);
ylim([0,25]);
subplot(1,3,2)
T2 = T(T.soma_center_1>center(1),:);
histogram(T2.bias_angle+90,12);
ylim([0,25]);
...
%%
for i = 1:1000
    indx1 = randperm(size(Tq1,1),round(size(Tq1,1)/2));
    indx2 = randperm(size(Tq2,1),round(size(Tq2,1)/2));
    indx3 = randperm(size(Tq3,1),round(size(Tq3,1)/2));
    indx4 = randperm(size(Tq4,1),round(size(Tq4,1)/2));
