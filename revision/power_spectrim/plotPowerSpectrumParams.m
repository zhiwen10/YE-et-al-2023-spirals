function [hr3e,hr3f] = plotPowerSpectrumParams(T,data_folder,save_folder)
...
hr3e = figure('Renderer', 'painters', 'Position', [100 100 300 300]);
subplot(1,1,1);
for kk = 1:size(T,1)
    %% session info
    mn = T.MouseID{kk};
...
print(hr3e, fullfile(save_folder,'FigR3e_power_diff'),'-dpdf', '-bestfit', '-painters');
%%
...
%%
hr3f = figure('Renderer', 'painters', 'Position', [100 100 300 300]);
subplot(1,1,1);
...
print(hr3f, fullfile(save_folder,'FigR3f_spiral_density_vs_bump_amp'),'-dpdf', '-bestfit', '-painters');
%%
