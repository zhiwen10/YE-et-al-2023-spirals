function hr1 = plotPlanarWave2(data_folder,save_folder)
...
quiver(posx,posy,vxRaw3_mean*5,vyRaw3_mean*5,'k');
hold on;
scatter(col_mean,row_mean,10,'w','filled');
hold on;
plot(positions(:,1),positions(:,2),'r');
...
%%
figure;
polarhistogram(angle_mean_border(15,:),25);
%%
figure;
ax1 = subplot(1,2,1);
load(fullfile(data_folder,'revision','plane_wave','angle_mean_all.mat'));
traceAmp_mean1 = traceAmp_mean_border;
angle_mean1 = angle_mean_border;
edges = [-pi:pi/6:pi];
n = 15;
for i = 1:n
    clear angle_mean N
    threshold = median(traceAmp_mean1(i,:));
    angle_mean = angle_mean1(traceAmp_mean1(i,:)<threshold);
    [N,edges] =histcounts(angle_mean,edges);
    N1(i,:) = N./size(angle_mean,2);
end
N_mean = mean(N1,1);
N_sem = std(N1,1)./sqrt(n);
