function plotSpiralSequence_freq3(T,mouseID,freq,data_folder,save_folder)
%% create save folder
...
% rawTrace = reshape(rawTrace,size(dV,2),size(Utransformed,1),size(Utransformed,2));
...
%%
rate1 = 1;
frameStart1 = 60385; frameEnd1 = 60935;
frameTemp = frameStart1-35-padding:frameEnd1+35+padding; % extra 2*35 frames before filter data 
V2 = double(V(:,frameTemp));

[trace2d1,traceAmp1,tracePhase1,rawTrace1] = spiralPhaseMap5_freq(Utransformed,V2,t,params,freq,rate1);
trace2d1 = trace2d1(:,:,1+35/rate1:end-35/rate1)./mimgtransformed; 
rawTrace = rawTrace1(:,:,1+35/rate1:end-35/rate1)./mimgtransformed; 
dff = trace2d1*100;
tracePhase1 = tracePhase1(:,:,1+35/rate1:end-35/rate1); % reduce 2*35 frames after filter data 
...
%
scale3 = 5/8;
t1 = t(frameStart1-padding:frameEnd1+padding);
tq = 1:rate1:numel(t1);
qt = interp1(1:numel(t1),t1,tq);
%
color2 = cbrewer2('seq','YlOrRd',9);
...
xlabel('Time (s)','fontSize',9);
line_ax1 = xline(t(frameStartAll(1)),'--');
line_ax2 = xline(t(frameStartAll(1))+1,'--');

line_ax3 = xline(t(frameStartAll(2)),'--');
line_ax4 = xline(t(frameStartAll(2))+1,'--');

hold on;
plot([t1(1)+0.5,t1(1)+0.5],[0,2],'r');
print(h2, fullfile(save_folder,...
    ['Spiral_sequence_' num2str(frameStart) '_' freq_name '.pdf']),...
    '-dpdf', '-bestfit', '-painters');
close all;
