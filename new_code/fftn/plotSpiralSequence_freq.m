function plotSpiralSequence_freq(T,mouseID,freq,data_folder,save_folder)
%% create save folder
...
for icell = 1:numel(groupedCells)
    ...
    if frameStart-35-padding>0
        ...
        %%
        [spirals_temp(:,1),spirals_temp(:,2)] = transformPointsForward(...
            tform,spirals_temp(:,1),spirals_temp(:,2));        
        spirals_temp2 = spirals_temp;
        spirals_temp2(:,5) = spirals_temp(:,5)-frameStart+1;
        spirals_temp2(:,1:2) = spirals_temp2(:,1:2)./params.downscale;
        %%
        ...
        for k = 1:frames
            ...
            hold on;
            plotOutline(maskPath([1:11]),st,atlas1,hemi,scale3,lineColor);
            indx = find(spirals_temp2(:,5)==k);
            if not(isempty(indx))
                hold on;
                scatter(spirals_temp2(indx,1),spirals_temp2(indx,2),10,'w','filled');
            end
