...
%%
h3 = figure('Renderer', 'painters', 'Position', [100 100 400 400]);
for i = 1:15
    psdx_mean = psdx_alpha_mean_all(:,i);
    nonalpha_psdx_mean = psdx_nonalpha_mean_all(:,i);
    subplot(4,4,i);
    freq_value = [0.5,2,4,6,8,10];
    log_freq_value = log10(freq_value);
    % freqN = 21;
    freqN = numel(freq1);
    plot(log10(freq1(2:freqN)),log10(psdx_mean(2:freqN)),'r');
    hold on;
    plot(log10(freq1(2:freqN)),log10(nonalpha_psdx_mean(2:freqN)),'k');
...
%%
for i = 1:15
    ratio(:,i) = log10(psdx_alpha_mean_all(:,i)./psdx_nonalpha_mean_all(:,i));
end
%%
ratio(ratio<0) = 0;
h3 = figure('Renderer', 'painters', 'Position', [100 100 400 400]);
...
for i = 1:15   
    ...
    % freqN = 21;
    freqN = numel(freq1);
    plot(log10(freq1(2:freqN)),ratio(2:freqN,i),'color',color1(i,:));
    hold on;
    % ylim([-7,-3]);
end
xlim([log_freq_value(1),log_freq_value(end)]);
