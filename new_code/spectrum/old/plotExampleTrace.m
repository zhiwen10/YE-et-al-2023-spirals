...
params.gsmooth = 0;
rate = 1;
frameStart = 10000; frameEnd = frameStart +3500;
frameTemp = frameStart-35:frameEnd+35; % extra 2*35 frames before filter data 
V1 = V(:,frameTemp);
dV1 = dV(:,frameTemp);
[trace2d1,traceAmp1,tracePhase1,rawTrace1] = spiralPhaseMap5(Utransformed1,V1,t,params,rate);
[trace2d1_dv,traceAmp1_dv,tracePhase1_dv,rawTrace1_dv] = spiralPhaseMap5(Utransformed1,dV1,t,params,rate);
freq = [2,8];
% [trace2d1,traceAmp1,tracePhase1,rawTrace1] = spiralPhaseMap5_freq(Utransformed1,V1,t,params,freq,rate);
trace2d1 = trace2d1(:,:,1+35/rate:end-35/rate)./mimgtransformed; 
rawTrace = rawTrace1(:,:,1+35/rate:end-35/rate)./mimgtransformed; 
trace2d1_dv = trace2d1_dv(:,:,1+35/rate:end-35/rate)./mimgtransformed; 
dff = trace2d1*100;
...
trace_filt_dv = zeros(7,size(trace2d1,3));
...
for i = 1:7
    ...
    trace_filt_dv(i,:) = trace2d1_dv(pixel(i,1),pixel(i,2),:);
...
%%
figure;
plot(t1,squeeze(trace_raw(3,:)),'k');
hold on;
plot(t1,squeeze(trace_filt(3,:)),'r');
hold on;
plot(t1,squeeze(trace_filt_dv(3,:)),'b');
%%
a = 35; b = 85; % height
c = 20; d = 70; % width
