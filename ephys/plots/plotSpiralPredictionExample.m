function [h4bc,h4de] = plotSpiralPredictionExample(T,data_folder,save_folder)
...
h4bc = figure('Renderer', 'painters', 'Position', [100 100 1000 600]);
ax1 = subplot(3,4,1);
...
xlim([1,t_sample]);
ylim([0,1]);
print(h4bc, fullfile(save_folder,['Fig4bc_' filename]),...
...
scale4 = 5/8;
frame_n = numel(frame);
h4de = figure('Renderer', 'painters', 'Position', [100 100 800 400]);
for i = 1:numel(frame)   
...
%%
print(h4de, fullfile(save_folder,['Fig4de_' filename '_frames']),...
    '-dpdf', '-bestfit', '-painters');